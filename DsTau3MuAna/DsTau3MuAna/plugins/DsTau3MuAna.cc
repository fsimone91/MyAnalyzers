...
class DsTau3MuAna : public edm::one::EDAnalyzer<edm::one::SharedResources>  {
 ...
 TH1F *   hVtxMass_KinVsGen;
  TH1F *   hVtxMass_RecoVsGen,  *hDPx_GenKal, *hDPy_GenKal, *hDPz_GenKal,  *hDPx_GenReco, *hDPx_GenKin;
...
DsTau3MuAna::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
   ...
   */
     ...
     bool mu1match = false;
     for (uint k=0; k<GenMu.size();k++){
       float dr1 = deltaR(GenMu.at(k).eta(),GenMu.at(k).phi(), TauIt->daughter(0)->eta(),TauIt->daughter(0)->phi());
       if (dr1<0.3 && dr1 < bestdr1) {
	 mu1match = true;
	 bestidx1 = k;
         bestdr1 = dr1;
       }
     ...
     //if(mu1match)     cout<<" bestdr1="<<bestdr1<<" k="<<bestidx1<<"mu1match "<<mu1match<<"  GenMu_x="<<GenMu.at(bestidx1).vx()<<endl;


     
     bool mu2match = false;     int bestidx2(-1);     float bestdr2(9999.);
     for (uint k=0; k<GenMu.size();k++){
       float dr2 = deltaR(GenMu.at(k).eta(),GenMu.at(k).phi(), TauIt->daughter(1)->eta(),TauIt->daughter(1)->phi());
       if (dr2<0.3 && dr2 < bestdr2) {
         mu2match = true;
         bestidx2 = k;
         bestdr2 = dr2;
       }
     }
     //if(mu2match) cout<<" bestdr2="<<bestdr2<<" k="<<bestidx2<<"mu2match "<<mu2match<<endl;

     bool mu3match = false;     int bestidx3(-1);     float bestdr3(9999.);
     for (uint k=0; k<GenMu.size();k++){
       float dr3 = deltaR(GenMu.at(k).eta(),GenMu.at(k).phi(), TauIt->daughter(2)->eta(),TauIt->daughter(2)->phi());
       if (dr3<0.3 && dr3 < bestdr3) {
         mu3match = true;
         bestidx3 = k;
         bestdr3 = dr3;
       }
     }
     //     if(mu3match) cout<<" bestdr3="<<bestdr3<<" k="<<bestidx3<<"mu3match "<<mu3match<<endl;          

     bool isTauMatch = (mu1match && mu2match && mu3match)  ;
     ...
     */
     ...
     GenMu1.SetPtEtaPhiM(GenMu.at(bestidx1).pt(), GenMu.at(bestidx1).eta(), GenMu.at(bestidx1).phi(), 0.10565837);
     GenMu2.SetPtEtaPhiM(GenMu.at(bestidx2).pt(), GenMu.at(bestidx2).eta(), GenMu.at(bestidx2).phi(), 0.10565837);
     GenMu3.SetPtEtaPhiM(GenMu.at(bestidx3).pt(), GenMu.at(bestidx3).eta(), GenMu.at(bestidx3).phi(), 0.10565837);
	...
	MatchedTripl2.push_back(*TauIt); 
       ...
       auto mu1 = TauVertexFitTree->currentParticle();
       vector<float> RefMu1=RefitMu(mu1);

       TauVertexFitTree->movePointerToTheNextChild();
       auto mu2 = TauVertexFitTree->currentParticle();
       vector<float> RefMu2=RefitMu(mu2);

       TauVertexFitTree->movePointerToTheNextChild();
       auto mu3 = TauVertexFitTree->currentParticle();
       vector<float> RefMu3=RefitMu(mu3);
     ...
     cout<<"mu1: px="<<TauIt->daughter(0)->px()<<" ref px="<<RefMu1.at(3)<<" GenMuPx="<<GenMu.at(bestidx1).px()<<endl;
     cout<<"mu2: px="<<TauIt->daughter(1)->px()<<" ref px="<<RefMu2.at(3)<<endl;
     cout<<"mu3: px="<<TauIt->daughter(2)->px()<<" ref px="<<RefMu3.at(3)<<endl;
     ...
     hDPx_GenKin->Fill(GenMu.at(bestidx1).px() - RefMu1.at(3));
     hDPx_GenKin->Fill(GenMu.at(bestidx2).px() - RefMu2.at(3));
     hDPx_GenKin->Fill(GenMu.at(bestidx3).px() - RefMu3.at(3));
       }
     }//Valid Kin Fit
     ...
     if(KVertex.hasRefittedTracks() && (bestidx1!=bestidx2 && bestidx2!=bestidx3 && bestidx1!=bestidx3)  ){
     ...
     cout<<"Mu1: GenPx="<<GenMu.at(bestidx1).px()<<" Kalman px="<<KalmanRefMu(ttrks.at(0)).at(3)<<" reco px="<<dau1->px()<<endl;
     cout<<"Mu2: GenPx="<<GenMu.at(bestidx2).px()<<" Kalman px="<<KalmanRefMu(ttrks.at(1)).at(3)<<" reco px="<<dau2->px()<<endl;
     cout<<"Mu3: GenPx="<<GenMu.at(bestidx3).px()<<" Kalman px="<<KalmanRefMu(ttrks.at(2)).at(3)<<" reco px="<<dau3->px()<<endl;
	 ...
	 pvTracks_refit2.push_back(pvTrack->second);}
       ...
       hDPx_GenKal->Fill(GenMu.at(bestidx1).px()-KMu1.Px());
       hDPx_GenKal->Fill(GenMu.at(bestidx2).px()-KMu2.Px());
       hDPx_GenKal->Fill(GenMu.at(bestidx3).px()-KMu3.Px());

       hDPy_GenKal->Fill(GenMu.at(bestidx1).py()-KMu1.Py());
       hDPy_GenKal->Fill(GenMu.at(bestidx2).py()-KMu2.Py());
       hDPy_GenKal->Fill(GenMu.at(bestidx3).py()-KMu3.Py());

       hDPz_GenKal->Fill(GenMu.at(bestidx1).pz()-KMu1.Pz());
       hDPz_GenKal->Fill(GenMu.at(bestidx2).pz()-KMu2.Pz());
       hDPz_GenKal->Fill(GenMu.at(bestidx3).pz()-KMu3.Pz());

       hDPx_GenReco->Fill(GenMu.at(bestidx1).px()-dau1->px());
       hDPx_GenReco->Fill(GenMu.at(bestidx2).px()-dau2->px());
       hDPx_GenReco->Fill(GenMu.at(bestidx3).px()-dau3->px());
   ...
   cout<<" Macthed triplets: "<<MatchedTripl.size()<<"  Arbitration="<<MatchedTripl2.size()<<endl;
...
DsTau3MuAna::beginJob()
{
  ...
  hVtxMass_KinVsCand =  fs->make<TH1F>("hVtxMass_KinVsCand","Three muons Mass_{KinFit}-Mass_{inv}",200,-20,20);
  hVtxMass_KinVsGen=  fs->make<TH1F>("hVtxMass_KinVsGen","Three muons Mass_{KinFit}-Mass_{gen}",200,-20,20);
  hVtxMass_RecoVsGen=  fs->make<TH1F>("hVtxMass_RecoVsGen","Three muons Mass_{reco}-Mass_{gen}",200,-20,20);
  ...
  hDPx_GenKal = fs->make<TH1F>("hDPx_GenKal", "px residuals(gen, kalman refit)", 200,-20,20);
  hDPx_GenKin = fs->make<TH1F>("hDPx_GenKin",  "px residuals(gen, kin refit)", 200,-20,20);
  hDPx_GenReco=fs->make<TH1F>("hDPx_GenReco", "px residuals(gen, reco)",200,-20,20);

  hDPy_GenKal  = fs->make<TH1F>("hDPy_GenKal", "py residuals(gen, kalman refit)", 200,-20,20);
  hDPz_GenKal = fs->make<TH1F>("hDPz_GenKal", "pz residuals(gen, kalman refit)", 200,-20,20);

  hDist_PVSV_Kin= fs->make<TH1F>("hDist_PVSV_Kin", "hDist_PVSV_Kin", 250, 0, 5);
  hDistErr_PVSV_Kin= fs->make<TH1F>("hDistErr_PVSV_Kin", "hDistErr_PVSV_Kin", 100, 0, 2);
  ...
  hDistErr_PVSV_Kal= fs->make<TH1F>("hDistErr_PVSV_Kal", "hDistErr_PVSV_Kal", 100, 0, 2);
  hDistComp_PVSV_Kal=fs->make<TH1F>("hDistComp_PVSV_Kal", "hDistComp_PVSV_Kal", 100, 0, 100);

  hVtxMass_KalVsCand = fs->make<TH1F>("hVtxMass_KalVsCand","Three muons Mass_{KinFit}-Mass_{inv}",200,-20,20);
  hVtxMass_KalVsGen= fs->make<TH1F>("hVtxMass_KalVsGen", "Three muons Mass_{KalFit}-Mass_{Gen}",200,-20,20);
  ...
  hValidVtx_size_Kalman = fs->make<TH1F>("hValidVtx_size_Kalman","hValidVtx_size_Kalman",1000,0,1);
